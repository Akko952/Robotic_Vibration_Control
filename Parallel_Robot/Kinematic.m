...
% 参数初始化-
S1(1).i = 1; 
S1(1).s_hat =rotate_vector(T_01,[0;1;0])/norm(rotate_vector(T_01,[0;1;0]));%与动平台连接处的旋转副轴为动坐标系的Y轴
% %通过旋转算子变成世界坐标系，由于不在意转轴的位置，只在意方向，因此只需要旋转【可能】
S1(1).ri=transform_Point_vector(T_01,p.b1); %转轴的位置为动平台与链路连接处，抽象出来一点,变换成世界坐标系
%关节旋量定义完成，开始组装TWIST
S1(1).nu =-vector_to_skew_symmetric(S1(1).s_hat)*S1(1).ri ;%叉乘算法，计算线速度部分
S1(1).xi=[S1(1).s_hat;...
    S1(1).nu];%旋转副的瞬时速度旋量
...
S1(2).ri = p.s1;
S1(2).nu = S1(2).s_hat;
S1(2).xi=[0;0;0;S1(2).nu];%移动副的瞬时速度旋量
...
S1(4).i = 4; S1(4).s_hat = S1(1).s_hat; S1(4).ri =p.s1;
...
S2(2).ri = p.s2;
S2(2).nu = S2(2).s_hat;
S2(2).xi=[0;0;0;S2(2).nu];%移动副的TWIST旋量
...
S2(4).i = 4; S2(4).s_hat = S2(1).s_hat; S2(4).ri =p.s2;
...
S3(2).ri = p.s3;
S3(2).nu = S3(2).s_hat;
S3(2).xi=[0;0;0;S3(2).nu];%移动副的TWIST旋量
...
S3(4).i = 4; S3(4).s_hat = S3(1).s_hat; S3(4).ri =p.s3;
